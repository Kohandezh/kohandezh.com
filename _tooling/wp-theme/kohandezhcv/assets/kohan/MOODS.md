...
| Walking patrol | `running-right` → `running-left` | Two short visible steps out and two steps back using only the approved atlas. |

## Supplemental moods

The package also keeps dedicated approved artwork for wink, angry still, confused Vision mode, Colt Kodiak low-ready, Russian Roulette (a fictional, non-graphic six-frame camera-facing sequence), drag-annoyed, fall-scared, and wave-snap-smoke goodbye. These are declared in `moods.json` with native fallbacks. In the public website bundle the Russian Roulette mood uses `supplemental-source/chamber-game-camera.png`; the older `chamber-camera` name remains as a compatibility alias.

`moods.json` is the avatar integration manifest. The website runtime mounts Kohan in the CV assistant and exposes `window.KohanAvatar`.
